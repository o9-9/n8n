...
const ROUTING_DECISION_TREE_WITHOUT_ASSISTANT = `1. Is user asking a conversational question or chatting? → responder
   Examples: "what does this do?", "explain the workflow", "thanks", "why is this node failing?", "how do I set up Gmail credentials?", "what does this error mean?"
   Examples with selected nodes: "what does this node do?", "explain how it works"
...
const KEY_DISTINCTION_WITHOUT_ASSISTANT = `- "Why is this node failing?" = ERROR DEBUG = responder (explain the issue)
- "What does this workflow do?" = EXPLANATION = responder (generates explanation from context)
- "How do I set up OAuth?" = CREDENTIAL HELP = responder (answer the question)
- "Use [ServiceB] instead of [ServiceA]" = REPLACEMENT = discovery (new node type needed)
- "Change the [ServiceA] API key" = CONFIGURATION = builder (same node, different value)`;
...
export function buildSupervisorPrompt(options?: SupervisorPromptOptions): string {
	const enableAssistant = options?.enableAssistant ?? false;

	return prompt()
		.section('role', SUPERVISOR_ROLE)
		.section(
			'available_agents',
			enableAssistant ? AVAILABLE_AGENTS_WITH_ASSISTANT : AVAILABLE_AGENTS_WITHOUT_ASSISTANT,
		)
		.section(
			'routing_decision_tree',
			enableAssistant
				? ROUTING_DECISION_TREE_WITH_ASSISTANT
				: ROUTING_DECISION_TREE_WITHOUT_ASSISTANT,
		)
		.section(
			'key_distinction',
			enableAssistant ? KEY_DISTINCTION_WITH_ASSISTANT : KEY_DISTINCTION_WITHOUT_ASSISTANT,
		)
